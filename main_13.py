...
bot = telebot.TeleBot('5612418710:AAGuvIy6_mQ-rOrPnMckg9ZCDx2wZl4M1FM')
moderId = 2089548580
...
def photo(message):
    if sended[message.chat.id] == 1:
        if message.photo:
            ...
            file_info = bot.get_file(file_id)
            downloaded_file = bot.download_file(file_info.file_path)
            bot.send_video(moderId, downloaded_file, caption="по квесту: " + nowQuest, reply_markup=keyboard, parse_mode="HTML")
            sended[message.chat.id] = 0
